# simulate_typing_verilog.py
import time,random,pyautogui,sys,signal
...
def biased_delay(min_delay,max_delay,bias_pow=3):
    r=random.random()
    biased=r**bias_pow
    delay=min_delay+(max_delay-min_delay)*biased
    delay*=1.0+random.uniform(-CHAR_JITTER,CHAR_JITTER)
...
def type_text(text):
    ...
    for ch in text:
        if skip_spaces:
            # Skip leading spaces/tabs right after newline
            if ch in [' ','\t']:
                continue
            else:
                skip_spaces=False
        ...
        pyautogui.write(ch)
        delay=biased_delay(MIN_DELAY,MAX_DELAY,BIAS_POW)
        if ch=='\n':
            skip_spaces=True
            time.sleep(delay+NEWLINE_EXTRA)
        else:
            time.sleep(delay)

def main():
    print(f"Buffering for {BUFFER_SECONDS}s. Focus your editor window now...")
    time.sleep(BUFFER_SECONDS)
    try:
        type_text(VERILOG_CODE)
    except pyautogui.FailSafeException:
        print("\nPyAutoGUI fail-safe triggered. Aborted.")
    except KeyboardInterrupt:
        print("\nKeyboardInterrupt received. Aborted.")
    else:
        print("\nDone typing.")
